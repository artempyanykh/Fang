...
and [<RequireQualifiedAccess>] Phrase =
...
and Expr =
    | Lit of L.BType
    | Var of Var
    // Captured variables can come from locals and env.
    | Closure of code: VarName * env: array<Var>
    // We don't do n-ary application just yet
...
type Builder(expr: L.Expr) =
    ...
    let rec buildFunction (ctx: Context) (name: VarName) (vars: List<VarName>) (body: L.Expr) : VarName =
        ...
        phrases.Add(Phrase.Function fn)
