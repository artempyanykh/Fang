...
module IntBinaryOp =
    let fromArithmeticFn (fn: L.BArithFn) =
        match fn with
        | L.Add -> IntBinaryOp.Add
        | L.Sub -> IntBinaryOp.Sub
        | L.Mul -> IntBinaryOp.Mul
        | L.Div -> IntBinaryOp.Div

    let fromComparisonFn (fn: L.CmpFn) =
        match fn with
        | L.Lt -> IntBinaryOp.Less
        | L.Eq -> IntBinaryOp.Equal
        | L.Gt -> IntBinaryOp.Greater
...
type BytecodeBuilder() =
    ...
    member this.Generate(m: Module) = ()

    member this.Generate(chunk: ChunkNum, expr: Expr) =
        match expr with
        | Lit (L.BType.Int i) -> this.EmitInstr(chunk, Instr.IntConst i)
        | Lit L.BType.Unit -> this.EmitInstr(chunk, Instr.IntConst 0)
